...
RSpec.describe "Resource planner split view navigation", :js, with_ee: %i[resource_planner_view] do
  include_context "with resource planner full access"

  let!(:view) { create(:view_resource_planner, query:) }
  ...
  it "allows to navigate to the split view" do
    ...
    resource_planner.within_lane(user) do
      resource_planner.expect_event work_package1
      resource_planner.expect_event work_package2
    end
    ...
    expect(page).to have_no_current_path /resource_planners\/new\/details\/#{work_package1.id}/

    # Open split view through info icon
    split_view = resource_planner.open_split_view_by_info_icon work_package1
    expect(page).to have_current_path /resource_planners\/new\/details\/#{work_package1.id}/
    ...
    expect(page).to have_current_path /resource_planners\/new\/details\/#{work_package2.id}/
