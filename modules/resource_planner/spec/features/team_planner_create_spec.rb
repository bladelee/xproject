...
RSpec.describe "Resource planner create new work package", :js, with_ee: %i[resource_planner_view] do
  ...
  shared_examples "can create a new work package" do
    it "creates a new work package for the given user" do
      ...
      split_create = resource_planner.add_item "/api/v3/users/#{user.id}",
      ...
      resource_planner.within_lane(user) do
        resource_planner.expect_event wp
      end
    end
  end
  ...
  context "with multiple users added" do
    ...
    before do
      ...
      resource_planner.expect_assignee(third_user, present: false)

      resource_planner.add_assignee other_user.name
      resource_planner.add_assignee third_user.name
      resource_planner.add_assignee user.name
    end
