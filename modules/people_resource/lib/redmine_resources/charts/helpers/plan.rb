      ...
      class Plan
        ...
        def build_booked_cards(date, is_workday, resource_bookings, time_entries)
          ...
          resource_bookings.inject([]) do |booked_cards, resource_booking|
            issue = resource_booking.work_package
        ...
        def find_booked_time_entries(resource_bookings, time_entries)
          booked_issue_ids = resource_bookings.map(&:work_package_id).compact
          booked_project_ids = resource_bookings.select { |rb| rb.work_package.blank? }.map(&:project_id)
