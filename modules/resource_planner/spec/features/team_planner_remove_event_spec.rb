...
RSpec.describe "Resource planner remove event",
               :js,
  ...
  let!(:other_user) do
    create(:user,
           ...
           member_with_permissions: { project => %w[
             view_work_packages view_resource_planner
           ] })
  end
  ...
  before do
    ...
    resource_planner.within_lane(other_user) do
      resource_planner.expect_event removable_wp
      resource_planner.expect_event non_removable_wp
    end
  ...
  it "can remove one of the work packages" do
    resource_planner.drag_to_remove_dropzone non_removable_wp, expect_removable: false
    resource_planner.drag_to_remove_dropzone removable_wp, expect_removable: true
  end
