...
RSpec.describe "Resource planner drag&dop and resizing",
               :js,
  ...
  let!(:other_user) do
    create(:user,
           firstname: "Bernd",
           member_with_permissions: { project => %w[view_work_packages view_resource_planner work_package_assigned] })
  end
  ...
  context "with full permissions" do
    before do
      ...
      resource_planner.within_lane(user) do
        resource_planner.expect_event first_wp, present: false
        resource_planner.expect_event second_wp, present: false
        resource_planner.expect_event third_wp
        resource_planner.expect_event fourth_wp
      end

      resource_planner.within_lane(other_user) do
        resource_planner.expect_event first_wp
        resource_planner.expect_event second_wp
        resource_planner.expect_event third_wp, present: false
        resource_planner.expect_event fourth_wp, present: false
      end
    end
    ...
    it "allows to drag&drop between the lanes to change the assignee" do
      ...
      retry_block do
        resource_planner.drag_wp_to_lane(first_wp, user)
      end
      resource_planner.expect_and_dismiss_toaster(message: I18n.t("js.notice_successful_update"))
      ...
      retry_block do
        resource_planner.drag_wp_to_lane(second_wp, user)
      end
      resource_planner.expect_and_dismiss_toaster(message: I18n.t("js.notice_successful_update"))
      ...
      retry_block do
        resource_planner.drag_wp_to_lane(third_wp, other_user)
      end
      resource_planner.expect_and_dismiss_toaster(message: I18n.t("js.notice_successful_update"))
      ...
      # Move the Milestone
      retry_block do
        resource_planner.drag_wp_to_lane(fourth_wp, other_user)
      end
      resource_planner.expect_and_dismiss_toaster(message: I18n.t("js.notice_successful_update"))
    ...
    it "allows to resize to change the dates of a wp" do
      retry_block do
        ...
        resource_planner.change_wp_date_by_resizing(second_wp, number_of_days: 1, is_start_date: true)
      end
      ...
      retry_block do
        resource_planner.drag_wp_by_pixel(second_wp, -150, 0)
      end
      resource_planner.expect_and_dismiss_toaster(message: I18n.t("js.notice_successful_update"))
      ...
      resource_planner.expect_wp_not_resizable(first_wp)
      # Elements that have start or due date outside of the current view are also not resizable
      resource_planner.expect_wp_not_resizable(third_wp)
      # Milestones are not resizable
      resource_planner.expect_wp_not_resizable(fourth_wp)

      # Instead we move the milestone completely to change the date
      retry_block do
        resource_planner.drag_wp_by_pixel(fourth_wp, 150, 0)
      end
      resource_planner.expect_and_dismiss_toaster(message: I18n.t("js.notice_successful_update"))
  ...
  context "without permission to edit" do
    ...
    it "allows neither dragging nor resizing any wp" do
      resource_planner.expect_wp_not_resizable(first_wp)
      resource_planner.expect_wp_not_resizable(second_wp)
      resource_planner.expect_wp_not_resizable(third_wp)
      resource_planner.expect_wp_not_resizable(fourth_wp)

      resource_planner.expect_wp_not_draggable(first_wp)
      resource_planner.expect_wp_not_draggable(second_wp)
      resource_planner.expect_wp_not_draggable(third_wp)
      resource_planner.expect_wp_not_draggable(fourth_wp)
    end
  end
end
