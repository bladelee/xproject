...
module Pages
  class ResourcePlanner < ::Pages::WorkPackageCards
    ...
    def path
      new_project_resource_planners_path(project)
    end

    def expect_title(title = "Unnamed resource planner")
    ...
    def expect_empty_state(present: true)
      expect(page).to have_conditional_selector(present,
                                                ".op-resource-planner--no-data",
                                                text: "Add assignees to set up your resource planner.")
    end

    def expect_view_mode(text)
      expect(page).to have_css('[data-test-selector="op-resource-planner--view-select-dropdown"]', text:)
    ...
    def switch_view_mode(text)
      retry_block do
        find('[data-test-selector="op-resource-planner--view-select-dropdown"]').click

        within("#op-resource-planner--view-select-dropdown") do
          click_button(text)
        end
      end
    ...
    def expect_delete_buttons_for(*queries)
      queries.each do |query|
        expect(page).to have_css "[data-test-selector='resource-planner-remove-#{query.id}']"
      end
    end

    def expect_no_delete_buttons_for(*queries)
      queries.each do |query|
        expect(page).to have_no_css "[data-test-selector='resource-planner-remove-#{query.id}']"
      end
    end
    ...
    def expect_create_button
      within ".toolbar-items" do
        expect(page).to have_link text: "Resource planner"
      end
    end

    def expect_no_create_button
      within ".toolbar-items" do
        expect(page).to have_no_link text: "Resource planner"
      end
    end
    ...
    def drag_to_remove_dropzone(work_package, expect_removable: true)
      ...
      footer = find('[data-test-selector="op-resource-planner-footer"]')
      drag_element_to(footer)

      sleep 1

      dropzone = find('[data-test-selector="op-resource-planner-dropzone"]')
      drag_element_to(dropzone)

      if expect_removable
        expect(page).to have_css("span", text: I18n.t("js.resource_planner.drag_here_to_remove"))
      else
        expect(page).to have_css("span", text: I18n.t("js.resource_planner.cannot_drag_here"))
      end
    ...
    def wait_for_loaded
      expect(page).to have_css(".op-resource-planner--wp-loading-skeleton")
      expect(page).to have_no_css(".op-resource-planner--wp-loading-skeleton", wait: 10)
    end
  end
end
