...
RSpec.describe API::V3::Views::ViewsAPI,
  ...
  let(:send_request) do
    post api_v3_paths.views_type("resource_planner"), body
  end
  ...
  describe "POST /api/v3/views/resource_planner" do
    context "with a user allowed to save the query" do
      ...
      it "returns the view" do
        expect(response.body)
          .to be_json_eql("Views::ResourcePlanner".to_json)
                .at_path("_type")
    ...
    context "with a user not allowed to manage resource planners" do
      let(:additional_setup) do
