      ...
      class BookedCardProgressBar < BaseComponent
        ...
        def render_log_time
          if User.current.allowed_to?(:log_time, @project)
            %(<td class="log-time">#{link_to I18n.t(:button_log_time), log_time_url, class: 'icon icon-time-add'}</td>).html_safe
          end
        end

        def render_progress_bar
          label = I18n.t(:label_resources_f_hour_short, value: to_int_if_whole(@spent_hours))
