module ::ResourcePlanner
  class ResourcePlannerController < BaseController
    ...
    menu_item :resource_planner_view
    ...
    def create
      ...
      if service_result.success?
        ...
        redirect_to project_resource_planner_path(@project, @view.query)
      else
        render action: :new
      end
    end
    ...
    def require_ee_token
      unless EnterpriseToken.allows_to?(:resource_planner_view)
    ...
    def create_service_class
      ResourcePlanner::Views::GlobalCreateService
    end
    ...
    def visible_plans(project = nil)
      query = Query
        ...
        .joins(:views)
        .references(:projects)
        .where("views.type" => "resource_planner")
        .order("queries.name ASC")

      if project
        query = query.where("queries.project_id" => project.id)
      else
        allowed_projects = Project.allowed_to(User.current, :view_resource_planner)
