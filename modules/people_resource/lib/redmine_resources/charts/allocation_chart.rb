...
class RedmineResources::Charts::AllocationChart 
  ...
  def render_button_add_booking
    @lines << view.content_tag(:div, I18n.t(:button_add),
        ...
        while i < user_line_map.length
          ...
          if day_plan && day_plan[:scheduled_hours] > 0
            ...
            if day_plan[:type_day] == DAYOFF
              ...
              bar_label = I18n.t(:label_resources_off)
            else
  ...
  def html_subject_content(object)
    ...
    when Issue
      ...
      if issue.assigned_to.present?
        assigned_string = I18n.t(:field_assigned_to) + ": " + issue.assigned_to.name
  ...
  def html_task_for_version(params, coords, _markers, label, _object)
    ...
    if label
      ...
      label = label.length > 1 ? I18n.t(:label_resources_versions) : label.first
