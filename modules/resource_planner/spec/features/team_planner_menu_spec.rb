...
RSpec.describe "Resource planner Menu Item", :js, :with_cuprite do
  shared_let(:project) do
    create(:project, enabled_module_names: %w[work_package_tracking resource_planner_view])
  end
  ...
  context "within the global menu" do
    ...
    context "when EE enabled", with_ee: %i[resource_planner_view] do
  ...
  context "within the project side menu" do
    context "as a user that does not have create rights" do
      ...
      context "when EE disabled" do
        it "does not show the create resource planner option" do
          ...
          expect(page).not_to have_test_selector("resource-planner--create-button")
        end
      end
    ...
    context "as a user that has create rights" do
      ...
      context "when EE enabled", with_ee: %i[resource_planner_view] do
        it "shows the create resource planner option" do
          visit project_path(project)
