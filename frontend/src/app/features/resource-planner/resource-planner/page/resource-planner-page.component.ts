...
export class ResourcePlannerPageComponent extends PartitionedQuerySpacePageComponent implements OnInit {
  @InjectField() actions$:ActionsService;

  text = {
    title: this.I18n.t('js.resource_planner.title'),
    unsaved_title: this.I18n.t('js.resource_planner.unsaved_title'),
  };
  ...
  /**
   * Reload the resource planner page if an external event was added.
   ...
   */
  @EffectCallback(resourcePlannerEventAdded)
  reloadOnEventAdded():void {
    void this.refresh(false, false);
  }

  refresh(visibly = false, _firstPage = false):void {
    this.actions$.dispatch(resourcePlannerPageRefresh({ showLoading: visibly }));
  }
}
