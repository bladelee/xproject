...
RSpec.describe "resource planner onboarding tour",
               :js,
               with_cuprite: false,
  ...
  let(:demo_project) do
    create(:project,
           ...
           public: true,
           enabled_module_names: %w[work_package_tracking gantt wiki resource_planner_view])
  end

  let(:user) do
    create(:admin,
           member_with_permissions: { demo_project => %w[view_work_packages edit_work_packages add_work_packages
                                                         view_resource_planner manage_resource_planner save_queries
  ...
  let(:query) { create(:query, user:, project: demo_project, public: true, name: "Resource planner") }
  let(:resource_plan) do
    create(:view_resource_planner,
           query:,
           assignees: [user],
  ...
  before do
    ...
    allow(Setting).to receive(:demo_view_of_type_resource_planner_seeded).and_return(true)
  end
  ...
  context "as a new user" do
    it "I see the resource planner onboarding tour in the demo project" do
      ...
      step_through_onboarding_resource_planner_tour
