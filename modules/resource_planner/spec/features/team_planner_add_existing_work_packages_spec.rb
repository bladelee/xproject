...
RSpec.describe "Resource planner add existing work packages", :js do
  ...
  context "with full permissions", with_ee: %i[resource_planner_view] do
    before do
    ...
    context "with a removable item" do
      ...
      it "shows work packages removed from the resource planner" do
        resource_planner.within_lane(user) do
          resource_planner.expect_event first_wp
        end
        ...
        # Remove task 1 from the resource planner
        resource_planner.drag_to_remove_dropzone first_wp, expect_removable: true
    ...
    it "allows to click cards to open split view when open" do
      ...
      # Open first wp
      split_screen = resource_planner.open_split_view_by_info_icon first_wp
    ...
    it "allows to add work packages via drag&drop from the left hand shortlist" do
      ...
      # Drag it to the resource planner...
      add_existing_pane.drag_wp_by_pixel second_wp, 800, 0

      resource_planner.expect_and_dismiss_toaster(message: "Successful update.")
      ...
      split_view = resource_planner.open_split_view_by_info_icon(third_wp)
      split_view.expect_open
    end

    it "the search applies the filter from the resource planner" do
      # Search for a work package
      add_existing_pane.search "Task"
      add_existing_pane.expect_result second_wp
      add_existing_pane.expect_result third_wp, visible: false

      # WP that are already shown in the resource planner are not shown again
  ...
  context "without permission to edit" do
    ...
    it "does not show the button to add existing work packages" do
      expect(page).not_to have_test_selector("op-resource-planner--add-existing-toggle")
