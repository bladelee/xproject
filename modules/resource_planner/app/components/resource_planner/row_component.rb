...
module ResourcePlanner
  class RowComponent < ::RowComponent
    ...
    def name
      link_to query.name, project_resource_planner_path(project, query.id)
    end
    ...
    def delete_link
      if table.current_user.allowed_in_project?(:manage_resource_planner, project)
        link_to(
          "",
          project_resource_planner_path(project, query.id),
          class: "spot-link icon icon-delete",
          method: :delete,
          data: {
            confirm: I18n.t(:text_are_you_sure),
            "test-selector": "resource-planner-remove-#{query.id}"
          },
