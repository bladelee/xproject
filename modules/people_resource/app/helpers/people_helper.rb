...
module PeopleHelper
  ...
  def people_tabs(person)
    tabs = [
      { name: 'activity', partial: 'activity', label: t(:label_activity)},
      { name: 'files', partial: 'attachments', label: t(:label_attachment_plural)},
      { name: 'projects', partial: 'projects', label: t(:label_project_plural)}
    ]

    tabs << { name: 'subordinates', partial: 'subordinates', label: t(:label_people_subordinates)} if person.try(:subordinates)
    tabs
  end
  ...
  def birthday_date(person)
    ...
    if person.birthday.day == Date.today.day && person.birthday.month == Date.today.month
      "#{t(:label_today).capitalize} #{"(#{ages})" unless ages.blank?}".strip
    else
  ...
  def people_list_styles_for_select
    list_styles = [[t(:label_people_list_excerpt), "list_excerpt"]]
  end

  def people_principals_check_box_tags(name, principals)
    s = ''
    principals.each do |principal|
      s << "<label>#{ check_box_tag name, principal.id, false, :id => nil } #{principal.is_a?(Group) ? t(:label_group) + ': ' + principal.to_s : principal}</label>\n"
    end
    s.html_safe
  end
  ...
  def people_principals_radio_button_tags(name, principals)
    s = ''
    principals.each do |principal|
      s << "<label>#{ radio_button_tag name, principal.id, false, :id => nil } #{principal.is_a?(Group) ? t(:label_group) + ': ' + principal.to_s : principal}</label>\n"
    end
    s.html_safe
  end

  def change_status_link(person)
    return unless  person.try(:id) != User.current.id && !person.try(:admin)
    url = {:controller => 'people', :action => 'update', :id => person, :page => params[:page], :status => params[:status], :tab => nil}

    if person.try(:locked)
      link_to t(:button_unlock), url.merge(:person => {:status => User::STATUS_ACTIVE}), :method => :put, :class => 'icon icon-unlock'
    elsif person.try(:registered)
      link_to t(:button_activate), url.merge(:person => {:status => User::STATUS_ACTIVE}), :method => :put, :class => 'icon icon-unlock'
    elsif person != User.current
      #link_to t(:button_lock), url.merge(:person => {:status => 3}), :method => :put, :class => 'icon icon-lock'
    end
  end
  ...
  def deviation_label(previous, current, options = {})
    ...
    result << "#{t(:label_previous)}: #{previous}\n#{t(:label_people_deviation)}: #{deviation}"
    result.html_safe
  end

  def label_period(period, date_format = '%m.%d')
    "#{t(:label_people_period)}: #{period.first.strftime(date_format)} - #{period.last.strftime(date_format)}"
  end

  def hours_with_minutes(time, label_hour = t(:label_people_hour), label_minute = t(:label_people_minute))
